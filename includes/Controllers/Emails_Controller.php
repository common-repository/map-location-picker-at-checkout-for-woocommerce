...
class Emails_Controller {
	...
	private function create_delivery_location_static_map( $order_id, $map_link, $latitude, $longitude ) {
		...
		?>
		<a href='<?php echo esc_attr( $map_link ); ?>' target='_blank'><img alt='Location Map' style='display: block !important; margin-bottom: 40px !important; margin-left: auto !important; margin-right: auto !important; position: relative !important;' src='<?php echo esc_attr( $image_src ); ?>' width='<?php echo esc_attr( $width ); ?>' height='<?php echo esc_attr( $height ); ?>'/></a>
		<?php
	}
