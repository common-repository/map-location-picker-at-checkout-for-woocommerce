...
class Frontend {
     ...
     * @since 1.0.0
     */
    private function setupGlobalJsVariables( $additional = array() ) : string {
        ...
        $store_locations_config = array(
            'use_selectWoo' => Store_Locations::enableSearchInStoreLocations(),
        );
        ...
        $store_locations_config = json_encode( $store_locations_config );
...
\t\tvar storeLocationsConfig = {$store_locations_config};
JAVASCRIPT;
    }
    ...
    public function outputCheckoutMap() {
        ...
        $instructions_text = get_option( 'lpac_map_instructions_text' );
        ...
        $global_js_vars = $this->setupGlobalJsVariables();
    ...
    public function lpac_output_map_on_order_details_page() {
        ...
        // Add inline global JS so that we can use data fetched using PHP inside JS
        $global_js_vars = $this->setupGlobalJsVariables( $collected_during_order );
        $added = wp_add_inline_script( LPAC_PLUGIN_NAME . '-base-map', $global_js_vars, 'before' );
        // On some websites our basemap might not enqueue in time. In those cases fall back to default wp jquery handle.
        if ( empty( $added ) ) {
            wp_add_inline_script( 'jquery-core', $global_js_vars, 'before' );
